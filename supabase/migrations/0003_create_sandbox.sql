...
-- 3) Copy seed tables
l_ddl_stmt := 'CREATE TABLE ' || DBMS_ASSERT.SIMPLE_SQL_NAME(p_username) || '.DEPARTAMENTOS AS SELECT * FROM SBX_OWNER.DEPARTAMENTOS';
EXECUTE IMMEDIATE l_ddl_stmt;
l_ddl_stmt := 'CREATE TABLE ' || DBMS_ASSERT.SIMPLE_SQL_NAME(p_username) || '.ESTUDIANTES AS SELECT * FROM SBX_OWNER.ESTUDIANTES';
EXECUTE IMMEDIATE l_ddl_stmt;
l_ddl_stmt := 'CREATE TABLE ' || DBMS_ASSERT.SIMPLE_SQL_NAME(p_username) || '.CURSOS AS SELECT * FROM SBX_OWNER.CURSOS';
EXECUTE IMMEDIATE l_ddl_stmt;
l_ddl_stmt := 'CREATE TABLE ' || DBMS_ASSERT.SIMPLE_SQL_NAME(p_username) || '.MATRICULAS AS SELECT * FROM SBX_OWNER.MATRICULAS';
EXECUTE IMMEDIATE l_ddl_stmt;
