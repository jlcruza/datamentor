...
INSERT INTO ai_system(token_limit) VALUES (10000);

CREATE OR REPLACE VIEW learning_content AS
       ...
       FROM modules AS m
         ...
         LEFT JOIN progress AS p ON p.lesson_id = l.lesson_id AND p.student_id = auth.uid()
       ORDER BY module_id, lesson_id ASC;
...
SELECT *
FROM ai_usage
WHERE billing_period >= date_trunc('month', timezone('UTC', now()))
  AND billing_period < (date_trunc('month', timezone('UTC', now())) + interval '1 month');
...
REVOKE ALL ON TABLE public.sandboxes FROM PUBLIC, anon, authenticated;
REVOKE ALL ON TABLE public.ai_system FROM PUBLIC, anon, authenticated;
...
GRANT SELECT ON TABLE public.ai_system TO authenticated;
...
-- RLS policies: ownership-based CRUD for ai_usage
CREATE POLICY "ai_usage select own rows" ON public.ai_usage
    FOR SELECT TO authenticated
        USING (student_id = auth.uid());

CREATE POLICY "ai_usage insert own rows" ON public.ai_usage
    FOR INSERT TO authenticated
       WITH CHECK (student_id = auth.uid());

CREATE POLICY "ai_usage update own rows" ON public.ai_usage
    FOR UPDATE TO authenticated
        USING (student_id = auth.uid())
        WITH CHECK (student_id = auth.uid());

CREATE POLICY "ai_usage delete own rows" ON public.ai_usage
    FOR DELETE TO authenticated
               USING (student_id = auth.uid());

-- DROP VIEW learning_content;
-- DROP VIEW practice_exercises;
