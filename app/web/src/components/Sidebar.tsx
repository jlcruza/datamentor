...
const Sidebar: React.FC<SidebarProps> = ({ activeSection, onSectionChange, lessons }) => {
  ...
  return (
    <>
      ...
      {/* Desktop / md+ sidebar */}
      <aside className="hidden md:block md:fixed md:left-0 md:top-16 md:h-[calc(100vh-4rem)] md:w-64 lg:w-72 bg-gray-800 border-r border-gray-700 overflow-y-auto">
        <nav className="p-4">
          <ul className="space-y-2">
            {menuItems.map((item) => {
              const isActive = activeSection === item.id;
